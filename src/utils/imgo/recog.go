...
func CosineSimilarityBytes(src1 , src2 []byte)(cossimi float64,err error){
	matrix1,err1:=ResizeForMatrixBytes(src1,80,60)
	if err1 != nil {
		err = err1
		return
	}

	matrix2,err2:=ResizeForMatrixBytes(src2,80,60)
	if err2 != nil {
		err = err2
		return
	}
