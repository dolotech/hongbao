...
func GetKey(addr string) string {
	...
	//glog.Info("Header: ",reqest.Header)
	//glog.Info("RequestURI:",reqest.RequestURI)
	...
	if err != nil { // 提交异常,返回错误
		...
		glog.Info("bodyerr: ",err,string(body))
		//glog.Info("body:",string(body))
		glog.Info("StatusCode:", resp.StatusCode)
		glog.Info("Location", resp.Header.Get("Location"))
		glog.Info("Header", resp.Header)
