...
func TestDSNServerPubKey(t *testing.T) {
	...
	cfg, err = ParseDSN(tst)
	if err == nil {
		t.Errorf("invalid name in DSN (%s) but did not error. Got cfg: %#v", tst, cfg)
	}
}
...
func TestDSNWithCustomTLS(t *testing.T) {
	...
	cfg, err := ParseDSN(tst)
	if err == nil {
		t.Errorf("invalid custom TLS in DSN (%s) but did not error. Got cfg: %#v", tst, cfg)
	}
...
func TestCloneConfig(t *testing.T) {
	...
	cfg2 := cfg.Clone()
	if cfg == cfg2 {
		t.Errorf("Config.Clone did not create a separate cfg struct")
	}
...
func TestNormalizeTLSConfig(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.tlsConfig, func(t *testing.T) {
			...
			if cfg.tls == nil {
				if tc.want != nil {
					t.Fatal("wanted a tls cfg but got nil instead")
				}
				return
			}
